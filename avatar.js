...
// MB-Lab Asian adult, in ordinary clothes; uniform world-space scale, not an arbitrary sprite.
export async function makeAvatar(scene){
 const gltf=await new GLTFLoader().loadAsync('./assets/asian-adult.glb?v=20260913-tour-film');
 ...
 const native={LeftArm:'upperarm_L',LeftForeArm:'lowerarm_L',LeftHand:'hand_L',RightArm:'upperarm_R',RightForeArm:'lowerarm_R',RightHand:'hand_R',LeftUpLeg:'thigh_L',LeftLeg:'calf_L',LeftFoot:'foot_L',RightUpLeg:'thigh_R',RightLeg:'calf_R',RightFoot:'foot_R'};
 for(const [alias,name] of Object.entries(native))if(bones[name])bones[alias]=bones[name];
 ...
 function stance(s=0){
  for(const [side,sign] of [['Left',1],['Right',-1]]){
   aim(side+'Arm',side+'ForeArm',sign*.13,-1,-s*sign*.45);
 ...
 const restY=model.position.y,footA=new T.Vector3(),footB=new T.Vector3();
 bones.LeftFoot.getWorldPosition(footA);bones.RightFoot.getWorldPosition(footB);const ankleY=Math.min(footA.y,footB.y);
 let phase=0,blend=0;
 group.visible=false;
 return {group,bones,height:1.65,scale,update(dt,moving,yaw){
  ...
  if(group.visible){model.position.y=restY;stance(Math.sin(phase)*.24*blend);group.updateMatrixWorld(true);bones.LeftFoot.getWorldPosition(footA);bones.RightFoot.getWorldPosition(footB);model.position.y+=ankleY-Math.min(footA.y,footB.y)+group.position.y;}
